...
def render_dashboard(company_id):
    ...
    # 2. UPDATED PANDAS RESAMPLE AGGREGATIONS
    ...
    if enable_ai:
        with st.spinner("AI is analyzing clinic historical trends..."):
            ...
            # 4. UPDATED AI FORECASTING TARGET METRIC
            plot_df = ai_forecasting.generate_forecast(
                trend_df, 
                date_col='visit_date', 
                metric_col='consultation_fee_ghs',
        ...
        # 5. UPDATED PLOTLY AREA CHART TARGET COLUMN
        fig_trend = px.area(
    ...
    # 6. UPDATED GROUPBY REVENUE CALCULATION
    treatment_revenue = df.groupby('treatment_type')['consultation_fee_ghs'].sum().reset_index()
    ...
    with col_right:
        # 7. UPDATED DONUT CHART VALUE TARGET
        fig_donut = px.pie(
            treatment_revenue, values='consultation_fee_ghs', names='treatment_type', hole=0.4,
